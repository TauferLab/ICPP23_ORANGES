...
#PBS -q prod 
#PBS -A Veloc
#PBS -o output/oranges_gpu_distributed64_polaris.out
#PBS -e output/oranges_gpu_distributed64_polaris.err
...
for i in $(seq 16); do
  ...
  echo "$hoststr" >> "./hostfile1"
done
for i in $(seq 17 32); do
  hoststr=$(sed "${i}q;d" $PBS_NODEFILE)
  echo "$hoststr" >> "./hostfile2"
done
for i in $(seq 5); do
  mpiexec -n ${NTOTRANKS} --ppn ${NRANKS_PER_NODE} --depth=8 --cpu-bind depth --hostfile "hostfile1" ../set_affinity_gpu_polaris.sh ../fido ../test_graphs/$GRAPH1 ../test_graphs/$GRAPH2 ../orbit_signatures.txt $TIME_DATA $INTERVAL 128 $MAX_INTERVAL $TIME_INTERVAL --run-full-chkpt --kokkos-num-threads=$NTHREADS --kokkos-map-device-id-by=mpi_rank &
#  mpiexec -n ${NTOTRANKS} --ppn ${NRANKS_PER_NODE} --depth=8 --cpu-bind depth --hostfile "hostfile2" ../set_affinity_gpu_polaris.sh ../fido ../test_graphs/$GRAPH1 ../test_graphs/$GRAPH2 ../orbit_signatures.txt $TIME_DATA $INTERVAL 128 $MAX_INTERVAL $TIME_INTERVAL --run-basic-chkpt --kokkos-num-threads=$NTHREADS --kokkos-map-device-id-by=mpi_rank &
  mpiexec -n ${NTOTRANKS} --ppn ${NRANKS_PER_NODE} --depth=8 --cpu-bind depth --hostfile "hostfile2" ../set_affinity_gpu_polaris.sh ../fido ../test_graphs/$GRAPH1 ../test_graphs/$GRAPH2 ../orbit_signatures.txt $TIME_DATA $INTERVAL 128 $MAX_INTERVAL $TIME_INTERVAL --run-tree-low-root-chkpt --kokkos-num-threads=$NTHREADS --kokkos-map-device-id-by=mpi_rank &
  wait
  mpiexec -n ${NTOTRANKS} --ppn ${NRANKS_PER_NODE} --depth=8 --cpu-bind depth --hostfile "hostfile1" ../set_affinity_gpu_polaris.sh ../fido ../test_graphs/$GRAPH1 ../test_graphs/$GRAPH2 ../orbit_signatures.txt $TIME_DATA $INTERVAL 128 $MAX_INTERVAL $TIME_INTERVAL --run-list-chkpt --kokkos-num-threads=$NTHREADS --kokkos-map-device-id-by=mpi_rank &
  mpiexec -n ${NTOTRANKS} --ppn ${NRANKS_PER_NODE} --depth=8 --cpu-bind depth --hostfile "hostfile2" ../set_affinity_gpu_polaris.sh ../fido ../test_graphs/$GRAPH1 ../test_graphs/$GRAPH2 ../orbit_signatures.txt $TIME_DATA $INTERVAL 128 $MAX_INTERVAL $TIME_INTERVAL --run-tree-low-offset-chkpt --kokkos-num-threads=$NTHREADS --kokkos-map-device-id-by=mpi_rank &
  wait
#  mpiexec -n ${NTOTRANKS} --ppn ${NRANKS_PER_NODE} --depth=8 --cpu-bind depth --hostfile "hostfile1" ../set_affinity_gpu_polaris.sh ../fido ../test_graphs/$GRAPH1 ../test_graphs/$GRAPH2 ../orbit_signatures.txt $TIME_DATA $INTERVAL 128 $MAX_INTERVAL $TIME_INTERVAL --run-tree-low-root-chkpt --kokkos-num-threads=$NTHREADS --kokkos-map-device-id-by=mpi_rank &
#  wait
done
