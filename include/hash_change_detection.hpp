...
void generate_hashes(GraphletDegreeVector gdv, Kokkos::View<uint64_t*>& hashes_d, size_t blocksize) {
  auto hashes = Kokkos::create_mirror_view(hashes_d);
  Kokkos::deep_copy(hashes_d, hashes);
...
//  }
  Kokkos::parallel_for("Generate hashes", Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(0,gdv.span()/blocksize_elements), KOKKOS_LAMBDA(const int i) {
    etl::murmur3<uint64_t> murmur3_64_gen;
    murmur3_64_gen.add((uint8_t*)(gdv.data()+(i*blocksize_elements)), (uint8_t*)(gdv.data()+((i+1)*blocksize_elements)));
    hashes(i) = murmur3_64_gen.value();
  });
  ...
  Kokkos::deep_copy(hashes, hashes_d);
