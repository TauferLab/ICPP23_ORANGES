...
echo ${results_path}
unq_job=$(date +%s)
sbatch -o ${output_file} -e ${error_file} -N 1 --wait -J fido_job -p ${slurm_queue} -n ${n_procs} -t ${slurm_time_limit} ${graph_align_job_script} ${n_runs} ${n_procs} ${input_graph1} ${input_graph2} ${load_assignment} ${results_path}
