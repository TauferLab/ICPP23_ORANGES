  ...
  while (queue.veclen > 0) {
    ...
    for (int i = 0; i < network.nodes_len; i++) {
      ...
      while (!is_neighbor && (neighbor_counter < network.vec[i].ListW.veclen)) {
        ...
        if (network.vec[i].ListW.vec[neighbor_counter].first == network.vec[current_node].Row) {
          is_neighbor = true;
        }
        neighbor_counter += 1;
      }
      ...
      while (!is_neighbor && (neighbor_counter < network.vec[current_node].ListW.veclen)) {
        ...
        if (network.vec[current_node].ListW.vec[neighbor_counter].first == network.vec[i].Row) {
          is_neighbor = true;
        }
        neighbor_counter += 1;
      }
