"""
    ...
    Author: Ali Khan
"""
# This file investigates chaining subtrees (pointing to parent) instead of storing entire subgraph every time
...
class Motif:
    def __init__(self, root: int, newedges: list[tuple[int,int]], endnodes: list[int], parent):
        self.root = root
        self.newedges = list(newedges)
        self.parent = parent
        self.endnodes = endnodes
        self.nodelist = self._nodes()
        self.num_nodes = len(self.nodelist)

    def add_edges(self,edges: list[tuple[int,int]]):
        self.newedges.extend(edges)
    ...
    def nodes(self) -> list[int]:
        # nodes = set()
        # for u,v in self.newedges:
        #     nodes.add(u)
        #     nodes.add(v)
        # if self.parent:
        #     return list(nodes.intersection(self.parent.nodes()))
        # else:
        #     return nodes
        # nodes = set()
        # for u,v in self.edgelist():
    ...
    def edgelist(self) -> list[tuple[int,int]]:
        ...
        return self.newedges + self.parent.edgelist()
...
def enumerate_subtrees(G: nx.Graph) -> dict[int : nx.Graph]:
  ...
  """
    ...
    for root in G.nodes():
        ...
        print("root:", root)
        # build ego graph
        # ego = nx.ego_graph(G,root,radius=MAX_NODES, center=True, undirected=True)
        ...
        for combo in leaf_combinations:
            ...
            T = Motif(root=root,newedges=edges, endnodes=nodes, parent=None)
            ...
            # tree_counts[tree_orbits[get_orbit(T,root)]] += 1
            # orbit_counts[get_orbit(T,root)] += 1
            orbit_counts[get_orbit2(T)] += 1
            ...
            if T.number_of_nodes() <= MAX_NODES:
                ...
                for combo in leaf_combinations:
                    ...
                    for edge in combo:
                        ...
                        endnodes.append(edge[0])


                    Tnew = Motif(root,newedges=combo,endnodes=endnodes,parent=T)
    ...
    # print(tree_counts)
    # return final_subtrees, time_vector, tree_counts
...
##### Orbit Matching #####
def get_orbit2(motif: Motif) -> int:
    G = nx.Graph()
    G.add_edges_from(motif.edgelist())
    return get_orbit(G, motif.root)
