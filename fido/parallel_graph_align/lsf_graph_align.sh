...
echo ${results_path}
unq_job=$(date +%s)
bsub -o ${output_file} -e ${error_file} -n ${n_procs} -m "tellico-compute0" -J "fidojobs_${unq_job}[1-${sims}]" ${graph_align_job_script} ${n_runs} ${n_procs} ${input_graph1} ${input_graph2} ${load_assignment} ${partition} ${results_path} -f input.\$LSB_JOBINDEX
